...
class FunctionDefinitionsTestCase(unittest.TestCase):
    ...
    def _subtest_nullary_function_definitions_no_body(self, funcname: str):
        for use_void in (False, True):
            check_subtest_single(self, self._define_empty_void_nullary_function,
                                 funcname, use_void)
            check_subtests(self, self._define_empty_nonvoid_nullary_function,
                           funcname, use_void)
    ...
    def _subtest_unary_function_definitions_no_body(self, funcname: str):
        for arr_specifier in '', '[]', '[][]', '[6]', '[42][]', '[SIZE]':
            check_subtest_single(self, self._define_empty_void_unary_function,
                                 funcname, arr_specifier)
            check_subtests(self, self._define_empty_nonvoid_unary_function,
                           funcname, arr_specifier)

    def test_no_body_function_definitions(self):
        self._subtest_nullary_function_definitions_no_body(SOME_IDENTIFIER)
        self._subtest_unary_function_definitions_no_body(SOME_IDENTIFIER)
